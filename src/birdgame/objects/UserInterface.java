...
public class UserInterface{
    ...
    */
    ...
    public void init(){
        ...
        uniFont20.getEffects().add(color);
        uniFont40.getEffects().add(color);
        uniFont60.getEffects().add(color);
        uniFont20.getEffects().add(outline);
        uniFont40.getEffects().add(outline);
        uniFont60.getEffects().add(outline);
    }
    ...
    private void showOverScreen(Graphics g){
        g.setColor(Color.white);
        g.setFont(uniFont40);
        g.drawString("SCORE", WIDTH/2 - g.getFont().getWidth("SCORE")/2, HEIGHT/3 + g.getFont().getLineHeight() - g.getFont().getLineHeight()/4);
        g.drawString(Stage.getPoints() + "", WIDTH/2 - g.getFont().getWidth(Stage.getPoints() + "")/2, HEIGHT/3 + 2*(g.getFont().getLineHeight() - g.getFont().getLineHeight()/4));
        g.drawString("BEST" , WIDTH/2 - g.getFont().getWidth("BEST")/2, HEIGHT/3 + 3*(g.getFont().getLineHeight() - g.getFont().getLineHeight()/4) + 10);
        g.drawString(Stage.getHighScore() + "", WIDTH/2 - g.getFont().getWidth(Stage.getHighScore() + "")/2, HEIGHT/3 + 4*(g.getFont().getLineHeight() - g.getFont().getLineHeight()/4) + 10);
        
        g.setColor(Color.white);
        g.setFont(uniFont20);
        g.drawString("PRESS SPACE TO RETRY...", WIDTH/2 - g.getFont().getWidth("PRESS SPACE TO RETRY...")/2, HEIGHT*2/3);
    }
}
