...
public class Stage{
    ...
    private void spawnPipes(){
        for(int i = 0; i < maxPipesNumber; i++){
            ...
            pipes[i][1] = new Rectangle(3*WIDTH/2 + i*(pipeWidth + gapX), pipes[i][0].getHeight() + gapY, pipeWidth, HEIGHT - gapY - pipes[i][0].getHeight() - Background.getGroundHeight());
    ...
    private void checkCollisions(){
        for(int i = 0; i < maxPipesNumber; i++){
            for(int j = 0; j < 2; j++){
                if(pipes[i][j].intersects(Bird.getRotatedHitbox()) || pipeHeads[i][j].intersects(Bird.getRotatedHitbox())){
                    if(points > highScore){
                        highScore = points;
                    }
                    if(Debug.isEnabled()){
    ...
    private void autoPilot(){
        for(int i = 0; i < maxPipesNumber; i++){
            if(pipes[i][1].getX() - Bird.getPositionX() < gapX && pipes[i][1].getX() - Bird.getPositionX() > -pipeWidth){
